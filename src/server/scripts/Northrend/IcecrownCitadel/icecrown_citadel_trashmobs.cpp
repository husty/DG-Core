            ...
            while(uint32 eventId = events.ExecuteEvent())
            {
                switch(eventId)
                {
                    ...
                    case EVENT_SABERLASH:
                        DoCastVictim(SPELL_SABERLASH, true);
    ...
    public:
        ...
        struct npc_valkyr_heraldAI : public ScriptedAI
        {
            ...
            void UpdateAI(uint32 uiDiff)
            {
                ...
                {
                    DoCastVictim(RAID_MODE(SPELL_SEVERED_ESSENCE_10N, SPELL_SEVERED_ESSENCE_25N, SPELL_SEVERED_ESSENCE_10N, SPELL_SEVERED_ESSENCE_25N));
            ...
            void UpdateAI(uint32 uiDiff)
            {
                ...
                if (m_uiMIRROR_Timer <= uiDiff)
                {
                    if (Unit* target = SelectTarget(SELECT_TARGET_RANDOM, 0))
                    {
                        ...
                        me->CastSpell(target,SPELL_BLOOD_MIRROR_DAMAGE,true);
                        me->CastSpell(me,SPELL_BLOOD_MIRROR_BUFF,true);
